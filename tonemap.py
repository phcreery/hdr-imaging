...
def plot_and_save(img, img_name, img_title):
    ...
    image_save = np.clip(img * 255, 0, 255).astype(np.uint8)
...
def local_tonemap(irradiance_map, img_name, saturation=1.0, gamma=1 / 2.2):
    ...
    local_tonemap = tonemap(
        irradiance_map, saturation=saturation, gamma_=gamma, numtiles=(8, 8)
    )
