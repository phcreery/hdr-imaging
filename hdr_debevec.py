...
def plot_crf(crf_channel, C, Zmax):
    ...
    for ch in range(C):
        ...
        mp_plt.plot(
            crf_channel[ch], np.arange(Zmax + 1), color=channel_names[ch], linewidth=2
        )
        mp_plt.xlabel("log(X)")
        mp_plt.ylabel("Pixel intensity")
        mp_plt.title("CRF for {} channel".format(channel_names[ch]))
    mp_plt.savefig(f"plot_crf_all.png")
    ...
    for ch in range(C):
        mp_plt.plot(
            crf_channel[ch],
            np.arange(Zmax + 1),
            color=channel_names[ch],
            linewidth=2,
            label=channel_names[ch] + " channel",
        )
    mp_plt.xlabel("log(X)")
    mp_plt.ylabel("Pixel intensity")
    mp_plt.title("Camera Response Function")

    mp_plt.legend()
    mp_plt.savefig("plot_crf.png")
